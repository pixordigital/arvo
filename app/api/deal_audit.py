...
async def create_audit(data: dict, background_tasks: BackgroundTasks, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), ctx=Depends(get_current_user)):
    ...
    key = idempotency_key or f"{org_id}:audit:{opp_id}:{uuid.uuid4()}"
    if key in _JOBS: return {"job_id": _JOBS[key]["job_id"], "status": _JOBS[key]["status"], "idempotency_key": key}
    job_id = str(uuid.uuid4())
    _JOBS[key] = {"job_id": job_id, "status": "QUEUED", "opportunity_id": opp_id, "org_id": org_id}
    background_tasks.add_task(_run_audit, job_id, key, opp_id, org_id, data.get("prompt_version","v1"))
...
async def _run_audit(job_id, key, opp_id, org_id, prompt_version):
    ...
    async with get_sessionmaker()() as s:
        ...
        if not opp or opp.org_id != org_id:
            _JOBS[key].update(status="FAILED", error="opportunity not found")
            return
        try:
            prov = get_provider()
            ...
            if result["exposure"] != "0.00":
                finding = Finding(org_id=org_id, kind="DEAL_RISK", title=f"Deal {opp.name} exposição {result['exposure']}", description=ai_res.get("content","")[:500], severity="HIGH" if result["materiality"]=="HIGH" else "MEDIUM", exposure_amount=result["exposure"], currency=opp.currency, confidence="MEDIUM", evidence_pack=result, idempotency_key=key)
                s.add(finding)
            await s.commit()
            _JOBS[key].update(status="DONE", result=result, extraction_id=extraction.id, decision_id=decision.id)
        except Exception as error:
            await s.rollback()
            _JOBS[key].update(status="FAILED", error=str(error))
